...
type DestinyProfileResponse struct {
	...
	// Records the timestamp of when most components were last generated from the world server
	// source. Unless the component type is specified in the documentation for
	// secondaryComponentsMintedTimestamp, this value is sufficient to do data freshness.
	ResponseMintedTimestamp time.Time `json:"responseMintedTimestamp"`

	// Some secondary components are not tracked in the primary response timestamp and have their
	// timestamp tracked here. If your component is any of the following, this field is where you will
	// find your timestamp value:
