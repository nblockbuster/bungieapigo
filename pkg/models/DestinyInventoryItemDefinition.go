...
type DestinyInventoryItemDefinition struct {
	...
	// DestinyItemComponent.tooltipNotificationIndexes property for which of these should be
	// shown at runtime.
	TooltipNotifications []DestinyItemTooltipNotification `json:"tooltipNotifications"`

	// If this item has a collectible related to it, this is the hash identifier of that collectible
	// entry.
	CollectibleHash int `json:"collectibleHash"`

	// If available, this is the original 'active' release watermark overlay for the icon. If the item
	// has different versions, this can be overridden by the 'display version watermark icon' from
	// the 'quality' block. Alternatively, if there is no watermark for the version, and the item
	// version has a power cap below the current season power cap, this can be overridden by the
	// iconWatermarkShelved property.
	IconWatermark string `json:"iconWatermark"`

	// If available, this is the 'shelved' release watermark overlay for the icon. If the item version
	// has a power cap below the current season power cap, it can be treated as 'shelved', and should be
	// shown with this 'shelved' watermark overlay.
	...
	// Confusing? Sure, that's why I call it "overlay" here: because as far as it's been used thus far,
	// it has been for an optional overlay image. We'll see if that holds up, but at least for now it
	// explains what this image is a bit better.
	SecondaryOverlay string `json:"secondaryOverlay"`

	// Pulled from the Secondary Icon, this is the "special" background for the item. For Emblems,
	// this is the background image used on the Details view: but it need not be limited to that for other
	// types of items.
	SecondarySpecial string `json:"secondarySpecial"`

	// Sometimes, an item will have a background color. Most notably this occurs with Emblems, who use
	// the Background Color for small character nameplates such as the "friends" view you see
	// in-game. There are almost certainly other items that have background color as well, though I
	// have not bothered to investigate what items have it nor what purposes they serve: use it as you
	// will.
	...
	// returned here. Note that not all items have screenshots: particularly not any non-equippable
	// items.
	...
	// time to really breakdown and maintain all the possible strings this could be, partly because
	// new ones could be added ad hoc. But if you want to use it to dictate your own UI, or look for items
	// with a certain display style, go for it!
	...
	// was more consistent. Many times, it has nothing. Sometimes, it's instead a more
	// narrative-forward description of the item. Which is cool, and I wish all properties had that
	// data, but it should really be its own property.
	...
	// performed when using the item. (Guess what? 99% of the time, this action is "dismantle".
	// Shocker)
	...
	// tracks PVP Kills, or what-have-you. This is a bit different from, for example, the Vanguard
	// Kill Tracker mod, which pipes data into the "art channel". When I get some time, I would like to
	// standardize these so you can get at the values they expose without having to care about what
	// they're being used for and how they are wired up, but for now here's the raw data.
	EmblemObjectiveHash int `json:"emblemObjectiveHash"`

	// If this item can be equipped, this block will be non-null and will be populated with the
	// conditions under which it can be equipped.
	...
	// consumed to get items from the box), this block will be non-null and will give summary
	// information for the items that can be acquired.
	...
	// default quality (item level, "quality", and infusion) data. See the block for more details,
	// there's often less upfront information in D2 so you'll want to be aware of how you use quality and
	// item level on the definition level now.
	...
	// If this item has a known source, this block will be non-null and populated with source
	// information. Unfortunately, at this time we are not generating sources: that is some
	// aggressively manual work which we didn't have time for, and I'm hoping to get back to at some
	// point in the future.
	...
	// If this item has related items in a "Gear Set", this will be non-null and the relationships
	// defined herein.
	Gearset DestinyItemGearsetBlockDefinition `json:"gearset"`

	// If this item is a "reward sack" that can be opened to provide other items, this will be non-null
	// and the properties of the sack contained herein.
	...
	// If the item has a Talent Grid, this will be non-null and the properties of the grid defined
	// herein. Note that, while many items still have talent grids, the only ones with meaningful
	// Nodes still on them will be Subclass/"Build" items.
	...
	// These investment stats don't take into account the ways that the items can spawn, nor do they
	// take into account any Stat Group transformations. I have retained them for debugging
	// purposes, but I do not know how useful people will find them.
	...
	// If the item has any related Lore (DestinyLoreDefinition), this will be the hash identifier you
	// can use to look up the lore definition.
	LoreHash int `json:"loreHash"`

	// There are times when the game will show you a "summary/vague" version of an item - such as a
	// description of its type represented as a DestinyInventoryItemDefinition - rather than
	// display the item itself.
	// This happens sometimes when summarizing possible rewards in a tooltip. This is the item
	// displayed instead, if it exists.
	...
	// The boolean will indicate to us (and you!) whether something *could* happen when you transfer
	// this item from the Postmaster that might be considered a "destructive" action.
	// It is not feasible currently to tell you (or ourelves!) in a consistent way whether this *will*
	// actually cause a destructive action, so we are playing it safe: if it has the potential to do so,
	// we will not allow it to be transferred from the Postmaster by default. You will need to check for
	// this flag before transferring an item from the Postmaster, or else you'll end up receiving an
	// error.
	...
	// In Destiny 1, we identified some items as having particular categories that we'd like to know
	// about for various internal logic purposes. These are defined in SpecialItemType, and while
	// these days the itemCategoryHashes are the preferred way of identifying types, we have
	// retained this enum for its convenience.
	SpecialItemType SpecialItemType `json:"specialItemType"`

	// A value indicating the "base" the of the item. This enum is a useful but dramatic
	// oversimplification of what it means for an item to have a "Type". Still, it's handy in many
	// situations.
	// itemCategoryHashes are the preferred way of identifying types, we have retained this enum for
	// its convenience.
	...
	// Some weapons and plugs can have a "Breaker Type": a special ability that works sort of like
	// damage type vulnerabilities. This is (almost?) always set on items by plugs.
	...
	// convenience. Whether you use the enum or hash and look up the definition depends on what's
	// cleanest for your code.
	...
	// but just in case weapons start being made that have multiple (for instance, an item where a
	// socket has reusable plugs for every possible damage type that you can choose from freely), this
	// field will return all of the possible damage types that are available to the weapon by default.
	...
	// Unfortunately, this does not preclude the possibility of something funky happening to give
	// the item a damage type that cannot be predicted beforehand: for example, if some designer
	// decides to create arbitrary non-reusable plugs that cause damage type to change.
	...
	// For various upsetting reasons, it's surprisingly cumbersome to figure this out. I hope you're
	// happy.
	...
	// I will likely regret leaving in the enumeration versions of these properties, but for now
	...
	// "wrapped" by one of these items so that we can safely provide refund capabilities before the
	// item is "unwrapped".
	IsWrapper bool `json:"isWrapper"`

	// Traits are metadata tags applied to this item. For example: armor slot, weapon type, foundry,
	// faction, etc. These IDs come from the game and don't map to any content, but should still be
	// useful.
